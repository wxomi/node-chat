...
type AIImageUpscalerNodeProps = {
  id: string;
  selected?: boolean;
  dragging?: boolean;
};

const AIImageUpscalerNode: React.FC<AIImageUpscalerNodeProps> = memo(
  ({ id, selected, dragging }) => {
    ...
    const deadzoneConfig = useMemo(() => {
      if (!flowConfig?.inputs || flowConfig.inputs.length === 0)
        return undefined;
      return {
        offset: 75, // Start at first handle
    ...
    return (
      <>
        ...
        <NodeAnimation>
          ...
          >
            {/* Node title */}
            <div className="text-body-desktop-medium mb-3">Upscale Image</div>
...
AIImageUpscalerNode.displayName = "AIImageUpscalerNode";

export default AIImageUpscalerNode;
