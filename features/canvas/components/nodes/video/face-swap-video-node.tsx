...
import FaceSwapVideoSourceMenuBar from "../../menu-bars/face-swap-video-source-menu-bar";
...
import { getVideoPreviewDimensions } from "../../../lib/shared";
...
type FaceSwapVideoNodeProps = {
  id: string;
  selected?: boolean;
  dragging?: boolean;
};

const FaceSwapVideoNode: React.FC<FaceSwapVideoNodeProps> = memo(
  ({ id, selected, dragging }) => {
    ...
    const nodeConfig = useConfigStore(
      (state) => state.nodeConfigs?.[id] as FaceSwapVideoNodeConfig | undefined
    );
    ...
    const videoSource = useConfigStore(
      (state) =>
        (state.nodeConfigs?.[id] as FaceSwapVideoNodeConfig | undefined)?.assets
          ?.videoSource || "file"
    );
...
FaceSwapVideoNode.displayName = "FaceSwapVideoNode";

export default FaceSwapVideoNode;
