...
const AnimationAudioMenuBar: React.FC<MenuBarProps> = ({
  options,
  activeOptionId,
  nodeId,
  onOptionClick,
    ...
    <div className="flex items-center gap-1.5 rounded-lg p-2 max-w-fit">
      {options.map((option) => {
        const currentConfig = nodeId
          ? useConfigStore.getState().nodeConfigs?.[nodeId]
          : undefined;
        const configAudioSource = currentConfig?.assets?.audioSource;
...
AnimationAudioMenuBar.displayName = "AnimationAudioMenuBar";

export default AnimationAudioMenuBar;
