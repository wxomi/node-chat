...
export async function handleVoiceGeneratePrototype(nodeId: string) {
  ...
  // Get typed prompt from embedded prompt (node data) or config
  const embeddedPrompt = (node?.data?.prompt as string) || "";
  const typedPrompt = embeddedPrompt || nodeConfig?.prompt || "";
      ...
      try {
        await poll({
          ...
          onError: (data: any) => {
            ...
            console.error(
              "[handleVoiceGeneratePrototype] Polling completed with error",
              {
          ...
          onCanceled: () => {
            ...
            console.warn("[handleVoiceGeneratePrototype] Polling canceled", {
              audioId: apiResult.id,
            });
