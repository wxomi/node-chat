...
export async function handleGenerateVideoPrototype(nodeId: string) {
  ...
  if (currentMode === "image-to-video") {
    ...
    // text-to-video: use embedded orientation from nodeData first, then config
    currentOrientation =
  ...
  try {
    ...
    switch (currentMode) {
      case "text-to-video": {
        // Get internal prompt from node data (embedded control)
        ...
        if (hasExternalConnection) {
          ...
          // No external connection - use internal prompt from nodeData, fallback to config
          const promptFromDataOrConfig =
            internalPrompt || nodeConfig.prompt || "";
          if (promptFromDataOrConfig.trim() !== "") {
            finalPrompt = promptFromDataOrConfig;
          } else {
        ...
        // Use embedded controls: nodeData first, then nodeConfig, then defaults
        const endSeconds =
          (nodeData?.endSeconds as number | undefined) ??
      ...
      case "image-to-video": {
        ...
        console.log(
          "[handleGenerateVideoPrototype] Retrieved image asset for image-to-video",
          {
      ...
      case "video-to-video": {
        ...
        console.log(
          "[handleGenerateVideoPrototype] Retrieved video asset for video-to-video",
          {
            sourceNodeId: edge.source,
            sourceNodeType: sourceNode.type,
        ...
        // Use embedded controls: nodeData first, then nodeConfig, then defaults
        const artStyle =
          (nodeData?.artStyle as string | undefined) ||
          nodeConfig.artStyle ||
          "No Art Style";
        const startSeconds =
          (nodeData?.startSeconds as number | undefined) ??
      ...
      try {
        await poll({
          ...
          onError: (data: any) => {
            ...
            console.error(
              "[handleGenerateVideoPrototype] Polling completed with error",
              {
          ...
          onCanceled: () => {
            ...
            console.warn("[handleGenerateVideoPrototype] Polling canceled", {
              videoId: apiResult.id,
            });
